       ...
       <form method="POST" action="{{ route('zones.update', $zone->id) }}">
        ...
        @method('PUT')
          ...
          <div class="col-md-6">
            ...
            <input type="number" class="form-control" id="latitude"  name="latitude" value="{{ $zone->latitude }}" readonly>
          </div>
          <div class="col-md-6">
            <label for="longitude" class="form-label">Longitude</label>
            <input type="number" class="form-control" id="longitude"  name="longitude" value="{{ $zone->longitude }}" readonly>
          </div>
          <div class="text-end">
...
document.getElementById('zone_name').addEventListener('change', function () {
    ...
    fetch('https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(zone)}')
        .then(res => res.json())
        .then(data => {
            if (data.length > 0) {
