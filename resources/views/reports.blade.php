...
.report-table thead.table-header {
    background: linear-gradient(90deg, #6362E7, #a471dfff);
    color: #fff;
...
.report-table thead {
  background: linear-gradient(90deg, #8c00ffff, #bf12ebff);
  color: #fff;
  text-transform: uppercase;
  font-size: 13px;
...
.report-table tbody tr:hover {
  background: linear-gradient(90deg, #d8a9ffff, #f3b1f5ff);
...
/* === Status Badges === */
.pending {
  background: linear-gradient(90deg, #aa140eff, #db4b5eff);
  color: #fff;
  font-weight: 600;
  padding: 6px 14px;
  border-radius: 30px;
...
.inprogress {
  background: linear-gradient(90deg, #ffb347, #fce410ff);
  color: #fff;
  font-weight: 600;
  padding: 6px 14px;
  border-radius: 30px;
...
.resolved {
  ...
  font-size: 13px;
  text-transform: capitalize;
  box-shadow: 0 2px 5px rgba(86,171,47,0.4);
}
...
<div class="sidebar-wrapper"> 
  ...
  <div>
    ...
    <!-- <div class="container">
    <h2 class="mb-4 text-center">📊 Complaint Reports</h2>
        ...
        <div class="row g-3 align-items-end">
            ...
            <div class="col-md-2">
                ...
                <select name="complaint_status" class="form-control">
                    ...
                    <option value="pending" {{ request('complaint_status')=='pending'?'selected':'' }}>Pending</option>
                     <option value="In progress" {{ request('complaint_status')=='Inprogress'?'selected':'' }}>In progress</option>
                    <option value="resolved" {{ request('complaint_status')=='Resolved'?'selected':'' }}>Resolved</option>
                </select>
            </div>
            ...
            <div id="reportContent" class="card shadow p-4">
                <h4 class="text-center mb-3">Complaint Report</h4>
                <table class="table table-bordered table-striped table-hover">
                    <thead style="background-color: #6362E7; color:white;">
                        <tr>
                            <th>Complaint_ID</th>
                            <th>Date</th>
                            <th>Ward</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @else
                                    <span class="badge bg-secondary">{{ ucfirst($complaint->complaint_status) }}</span>
                                @endif
                            </td>
    ...
    <div class="container my-5">
      <div class="text-center mb-4">
            <h3 class="text-muted">Reports</h3>
      </div>
    ...
    <!-- Filters form remains unchanged -->
    <form action="{{ route('reports') }}" method="GET" class="mb-4">
...
<div class="col-md-2">
    ...
    <!-- Styled Report Table -->
    <div id="reportContent" class="card shadow-lg p-4">
        <div class="text-center mb-4">
        <img src="{{ asset('images/logo.png') }}" alt="BBMP Logo" style="width: 80px; margin-bottom: 15px;">
        <h2 class="fw-bold">Black Spot Monitoring System</h2>
        <h3 class="text-muted">Complaint Reports</h3>
        </div>
        <!-- <h4 class="text-center mb-3">Complaint Report</h4> -->
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            @else
                                <span class="inprogress">{{ ucfirst($complaint->complaint_status) }}</span>
                            @endif
                        </td>
                        <td>{{ $complaint->remarks ?? '—' }}</td>
                        <td>
                        <img src="{{ asset('complaint_images/'.$complaint->image) }}" 
                            alt="Complaint Image" 
                            class="img-fluid rounded shadow mt-2" style="max-width:60px;">
                        </td>

                        <!-- <td>
                            @foreach($complaint->officials as $official)
                                <span class="">{{ $official->name }}</span>
                            @endforeach
                        </td> -->
                    </tr>
                    @empty
...
document.getElementById("downloadPdf").addEventListener("click", function () {
    ...
    // 1. Disable To Date initially
    toDate.disabled = true;

    // 2. Set today's date as max allowed for From Date
    let today = new Date().toISOString().split("T")[0];
    fromDate.max = today;

    fromDate.addEventListener('change', function () {

        // Validation: From Date cannot be in the future
        ...
        // Enable To Date and set minimum valid date
        if (fromDate.value) {
        ...
        // Validation: If To Date < From Date → reset
