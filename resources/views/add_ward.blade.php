      ...
      <div class="row">
        <div class="col-sm-12">
          <div class="card">
            ...
            <div class="card-body">
              <form class="row needs-validation " novalidate=""  method="POST" action="{{ route('wards.store') }}">
                @csrf
                 <div class="col-sm-6">
                  <div class="mb-3">
                    <label for="division">Division</label>
                    <!-- <input class="form-control" id="division" type="text" name="division_name" placeholder="Enter Division" required value=""> -->
                    <select name="division_name" id="division" class="form-control" required>
                ...
                <div class="col-sm-6">
                  <div class="mb-3">
                    ...
                    <input class="form-control" id="wname" type="text" name="ward_name" placeholder="Enter Ward Name" required value="">
                    <!-- <select name="ward_name" id="ward" class="form-control" required>
                ...
                <div class="col-sm-6">
                  <div class="mb-3">
                    ...
                    <input class="form-control" id="wno" type="number" name="ward_no" placeholder="Enter ward number" required value="">
...
$('#division').on('change', function() {
    ...
    if(division){
        $.getJSON("/wards/get-wards/" + division, function(data){
            $('#ward').empty().append('<option value="">Select Ward</option>');
            $('#ward_no').empty().append('<option value="">Select Ward No</option>');
            $.each(data, function(index, ward){
                $('#ward').append('<option value="'+ ward.name +'">'+ ward.name +'</option>');
                $('#ward_no').append('<option value="'+ ward.no +'">'+ ward.no +'</option>');
            });
        });
    }
});
...
$('#division').on('change', function() {
    ...
    if (division) {
        $.getJSON("/wards/get-wards/" + division, function(data) {
            ...
            $.each(data, function(index, ward) {
                // store ward.no inside option's data attribute
                $('#ward').append('<option value="'+ ward.name +'" data-no="'+ ward.no +'">'+ ward.name +'</option>');
            });
        });
    }
});

// when user selects ward, update ward_no dropdown
$('#ward').on('change', function() {
    let selected = $(this).find(':selected');
    let wardNo = selected.data('no');

    $('#ward_no').empty().append('<option value="">Select Ward No</option>');

    if (wardNo) {
        $('#ward_no').append('<option value="'+ wardNo +'">'+ wardNo +'</option>');
    }
});
</script>
