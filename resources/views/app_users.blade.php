          ...
          <table class="display table table-striped" id="divisionTable" style="width:100%">
            ...
            <tbody>
              ...
              <tr>
                  ...
                  <td>{{ $user->address }}</td>
                  <td>
                    <button class="btn btn-sm btn-info show-complaints-btn" 
                            data-user="{{ $user->name }}" 
                            data-complaints='@json($user->complaints)'>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header bg-primary text-white">
          <h5 class="modal-title" id="complaintsModalLabel">User Complaints</h5>
          <button type="button" class="btn btn-sm btn-light me-2" id="downloadPdfBtn">Download PDF</button>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body" id="complaintsModalBody">
        <!-- Complaints details will be injected here -->
      </div>
    </div>
  </div>
</div>
...
$(document).ready(function(){
    ...
    $('.show-complaints-btn').click(function(){
        var userName = $(this).data('user');
        var complaints = $(this).data('complaints');

        var html = `<h6>Complaints of ${userName}</h6>`;
        html += `<table class="table table-bordered table-striped mt-2">
                    ...
                    <tbody>`;
        complaints.forEach(function(c, index){
            html += `<tr>
                        <td>${index+1}</td>
                        <td>${c.id}</td>
        ...
        $('#complaintsModalBody').html(html);
        var modal = new bootstrap.Modal(document.getElementById('complaintsModal'));
        modal.show();
    });
...
$(document).ready(function(){
    ...
    $('.show-complaints-btn').click(function(){
        ...
        var html = `<h6>Complaints of ${userName}</h6>`;
        html += `<table class="table table-bordered table-striped mt-2" id="complaintsTable">
                    <thead class="table-primary">
                        <tr>
                            <th>#</th>
                            <th>Complaint ID</th>
        ...
        $('#complaintsModalBody').html(html);
        currentModalBody = html; // store current content for PDF
        var modal = new bootstrap.Modal(document.getElementById('complaintsModal'));
        modal.show();
    });

    $('#downloadPdfBtn').click(function(){
        if(!currentModalBody) return;

        html2canvas(document.querySelector("#complaintsModalBody")).then(canvas => {
